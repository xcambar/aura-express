  ...
  return function(app) {
    ...
    Widget.loadWidgetDefinition = function (ref) {
      ...
      require([ref], function (widgetDefinition) {
        ...
        core.data.when(widgetDefinition).then(function(def) {
          try {
            var WidgetConstructor = buildWidgetConstructor(ref, def);
    ...
    return {
      ...
      init: function (app) {
        ...
        app.core.Widgets.extend = function (name, ref, extendedProperties) {
          var splitRef = ref.split('@'),
              source = splitRef[1] || "default",
              basename = splitRef[0];

          ref = "__widget__$" + [basename, source].join('@');
          var dfd = app.core.data.deferred();
          
          var subWidgetName = "__widget__$" + name + "@default";

          if (require.defined(ref)) {
            console.log("PARENT DEFINED", ref);
            dfd.resolve(_.extend({}, require(ref), extendedProperties));
          } else {
            console.log("PARENT UNDEFINED", ref);
            var widgetsPath    = app.config.widgets.sources[source] || "widgets";
            var requireConfig = { packages: [ { name: ref, location: widgetsPath + "/" + basename } ]};
            require.config(requireConfig);
            require([ref], function (parentModule) {
              console.log("DEFINING PARENT", parentModule);
              dfd.resolve(_.extend({}, parentModule, extendedProperties));
            });
          }
          return core.data.when(dfd).then(function (mod) {
            console.log("WHEN IS FINISHED", mod);
            define(subWidgetName, [ref], function (newModule) {
              console.log("MODULE DEFINED", mod, newModule);
              return mod;
            });
            return mod;
          }).promise();
